      ...
      <div className="panel w-full max-w-lg p-8 md:p-10 rounded-[2.5rem] bg-bg-panel/40 border-white/5 shadow-2xl animate-pop-in relative overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl font-black text-white italic tracking-tight uppercase">
            Create <span className="text-brand-secondary">Room</span>
          </h2>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex bg-bg-main p-1 rounded-2xl border border-white/5">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-[10px] font-black transition-all ${isPrivate ? 'bg-rose-500/20 text-rose-400 shadow-lg border border-rose-500/20' : 'text-slate-600'}`}
              >
                <span>🔒</span> PRIVATE
              </button>
              <button
                onClick={() => setIsPrivate(false)}
                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-[10px] font-black transition-all ${!isPrivate ? 'bg-brand-secondary text-bg-main shadow-lg' : 'text-slate-600'}`}
              >
                <span>🌍</span> PUBLIC
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              ...
              <input
                type="range" min="2" max="5" value={wordCount}
          ...
          {/* Word Mode */}
          <div className="space-y-3">
            <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Word Mode</label>
            <select
              value={wordMode}
          ...
          <button
            ...
            disabled={loading}
            className={`w-full py-5 rounded-2xl font-black text-[11px] tracking-[0.4em] btn-game shadow-tactile-heavy transition-all mt-2 ${loading ? 'bg-brand-primary/50 text-white cursor-not-allowed animate-pulse' : 'bg-brand-primary hover:bg-brand-primary/90 text-white'}`}
          >
